...
class Blockchain(Base):
    ...
    represents a blockchain supported by Vision.

    Attributes
    ----------
    id : sqlalchemy.Column
        The unique blockchain ID (primary key, equal to the
        vision.common.blockchains.Blockchain enum value).
    name : sqlalchemy.Column
        The blockchain's name (equal to the
        vision.common.blockchains.Blockchain enum name).
    last_block_number : sqlalchemy.Column
        The last block monitored for new Vision TransferFromSucceeded
...
class HubContract(Base):
    ...
    instance represents a Vision Hub contract deployed on a blockchain.
    It can be either the current or an old version of the Vision Hub
...
class ForwarderContract(Base):
    ...
    instance represents a Vision Forwarder contract deployed on a
    blockchain. It can be either the current or an old version of the
    Vision Forwarder contract on its blockchain.
...
class TokenContract(Base):
    ...
    blockchain and registered at the Vision Hub.
...
class ValidatorNode(Base):
    ...
    registered at the Vision Forwarder contract of a supported
    blockchain.
...
class TransferStatus(Base):
    ...
    instance represents a possible status of Vision transfers.

    Attributes
    ----------
    id : sqlalchemy.Column
        The unique transfer status ID (primary key, equal to the
        vision.validatornode.database.enums.TransferStatus enum
        value).
    name : sqlalchemy.Column
...
class Transfer(Base):
    ...
    represents a cross-chain Vision token transfer.
    ...
    validator_nonce : sqlalchemy.Column
        The nonce of the validator at the Vision Forwarder contract of
        the destination blockchain.
    source_hub_contract_id : sqlalchemy.Column
        The unique ID of the Vision Hub contract on the source
        blockchain (foreign key).
    destination_hub_contract_id : sqlalchemy.Column
        The unique ID of the Vision Hub contract on the destination
        blockchain (foreign key).
    destination_forwarder_contract_id : sqlalchemy.Column
        The unique ID of the Vision Forwarder contract on the
    ...
    source_transfer_id : sqlalchemy.Column
        The unique transfer ID assigned by the Vision Hub contract of
