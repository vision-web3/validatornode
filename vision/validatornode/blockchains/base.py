...
from vision.validatornode.exceptions import ValidatorNodeError
from vision.validatornode.protocol import is_supported_protocol_version
...
class NonMatchingForwarderError(BlockchainClientError):
    """Exception to be raised if the Vision Forwarder of a transferred
    token does not match the Vision Forwarder of the Vision Hub on the
...
class SourceTransferIdAlreadyUsedError(BlockchainClientError):
    ...
    submitted with a source blockchain Vision transfer ID that was
...
class BlockchainClient(BlockchainHandler, ErrorCreator[BlockchainClientError]):
    ...
    def read_minimum_validator_node_signatures(self) -> int:
        """Read the minimum number of required Vision validator node
    ...
    class ReadOutgoingTransfersFromBlockResponse:
        """Response data for reading the outgoing Vision transfers from
        a starting block.
    ...
    def read_outgoing_transfers_from_block(self, from_block_number: int) -> \
        ...
        """Read the outgoing Vision transfers included in blocks
    ...
    def read_outgoing_transfers_in_transaction(
        ...
        """Read the outgoing Vision transfers included in a specified
        transaction.
        ...
        hub_address : BlockchainAddress
            The address of the Vision Hub contract the outgoing
    ...
    def read_validator_node_addresses(self) -> list[BlockchainAddress]:
        """Read the addresses of all Vision validator nodes registered
        on the blockchain.
    ...
    class TransferToSignerAddressRecoveryRequest:
        ...
        validator_nonce : int
            The nonce of the validator nodes at the Vision Forwarder
    ...
    def recover_transfer_to_signer_address(
            ...
            -> BlockchainAddress:
        """Recover the signer's address from a Vision transferTo
        signature.
    ...
    class TransferToMessageSignRequest:
        """Request data for signing a Vision transferTo message.
        ...
        destination_hub_address : BlockchainAddress
            The address of the Vision Hub contract on the (eventual)
            destination blockchain.
        destination_forwarder_address : BlockchainAddress
            The address of the Vision Forwarder contract on the
    ...
    def sign_transfer_to_message(self,
        ...
        """Sign a Vision transferTo message.
    ...
    def start_transfer_to_submission(
        ...
        NonMatchingForwarderError
            If the Vision Forwarder of the transferred token does not
            match the Vision Forwarder of the Vision Hub on the
            destination blockchain.
        SourceTransferIdAlreadyUsedError
            If the source blockchain's Vision transfer ID was already
    ...
    class TransferToSubmissionStatusResponse:
        ...
        destination_transfer_id : int or None
            The Vision transfer ID on the (eventual) destination
    ...
    def _read_transfer_to_transaction_data(
        ...
        read_destination_transfer_id : bool
            True if the Vision transfer ID on the (eventual) destination
